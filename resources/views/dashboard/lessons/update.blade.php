      ...
      <div class="container-fluid">
        <div class="row">
          ...
          <div class="col-md-12">
            ...
            <!-- /.card-header -->
            <!-- form start -->
            <form id="form" role="form" method="POST" action="{{ route('dashboard.lessons.update', ['lesson'=>$lesson->id]) }}" enctype="multipart/form-data">
              @csrf
              @method('put')
              @if ($errors->any())
              ...
              @endif
                ...
                <div class="card-body">
                    <div class="form-group">
                        ...
                        <input value="{{ $lesson->name }}" name="name" type="text" class="form-control" id="exampleInputEmail1" placeholder="نامدرس را وارد کنید">
                      </div>
                  ...
                  <div class="form-group">
                    ...
                    <label for="exampleInputEmail1">مدرسه</label>
                      <select name="school_id" class="form-control" id="">
                        <option selected value="{{ $lesson->school_id }}">{{ $lesson->school->name }}</option>
                        @foreach ($schools as $school)
                        @if ($school->id == $lesson->school_id)
                    ...
                    <div  class="form-group" id="divClass">
                      ...
                      <option selected value="{{ $lesson->class_id }}">{{ $lesson->class->name }}</option>
                        ...
                        <option hidden value="{{ $class->id }}">{{ $class->name }}</option>
                        @else
                  ...
                  <div class="form-group" id="divTeacher">
                    ...
                    <select name="teacher_id" id="teacher" class="select-picker1 form-control" data-live-search="true">
                      <option selected value="{{ $lesson->teacher_id }}">{{ $lesson->teacher->name }}</option>
                    @foreach ($teachers as $teacher)
                        @if ($teacher->id == $lesson->teacher_id)
                        <option hidden value="{{ $teacher->id }}">{{ $teacher->name }}</option>
                        @else
...
<script>
  ...
  new Chart("myChart1", {
    ...
    options: {
      title: {
        display: true,
        text: "بیشترین غایبین موجود دردرس  ها"
      }
    }
  });
  </script>
