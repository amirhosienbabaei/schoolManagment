      ...
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-8" style="display: flex;align-items: center;justify-content: space-between">
            <h1 class="m-0 text-dark">ویرایش مورد انضباطی</h1>
            <p>
          ...
          <div class="col-sm-12">
            <ol class="breadcrumb float-sm-left">
              ...
              <li class="breadcrumb-item active">ویرایش مورد انضباطی</li>
            </ol>
          </div>
      </div><!-- /.container-fluid -->
    </section>
      ...
      <div class="container-fluid">
        <div class="row">
          ...
          <div class="col-md-12">
            ...
            <!-- /.card-header -->
            <!-- form start -->
            <form id="form" role="form" method="POST" action="{{ route('dashboard.disciplineExamps.update', ['disciplineExamp'=>$data->id]) }}" enctype="multipart/form-data">
              @csrf
              @method('put')
              
              @if (Auth::user()->type == 1)
              <select name="school_id"   id="school" class="form-control col-12 mt-1">
                <option value="{{ $data->student->school->id }}" selected>{{ $data->student->school->name }}</option>
                @foreach ($schools as $school)
                @if ($school->id == $data->student->school->id)
                @else
              ...
              <select name="class_id"   id="class_main" class="form-control select-picker col-12 mt-3">
                <option value="{{ $data->student->class->id }}" selected>{{ $data->student->class->name }}</option>
                @foreach ($classes as $class)
                @if ($class->id == $data->student->school->id)
                @else
              ...
              <select name="student_id"   id="student_main" class="form-control select-picker1 col-12 mt-3">
                <option value="{{ $data->student->id }}" selected>{{ $data->student->name }}</option>
                @foreach ($students as $student)
                @if ($student->id == $data->student->school->id)
                @else
              ...
              <select  name="dismissal_status" id="student_main" class="form-control select-picker1 col-12 mt-3">
              ...
              <input type="text" class="form-control mt-3"  name="dismissal_count" value="{{ $data->dismissal_count }}" placeholder="در صورت نیاز به اخراج تعداد روز اخراج را وارد نمایید">



              <input type="text"   value="{{ $data->title }}" class="form-control mt-3" name="title" placeholder="موضوع را وارد کنید">

              <input type="number" value="{{ $data->score }}"   class="form-control mt-3" name="score" placeholder="نمره ای که قرار است از انضباط کم شود را وارد کنید">
              <label class="mt-2" for="discription">خلاصه ای در مورد بی نظمی توضیح دهید</label>
              <textarea   class="form-control mt-3" name="discription" id="discription" cols="5" rows="5">
                {{ $data->discription }}
              </textarea>
...
<script>
  ...
  new Chart("myChart1", {
    ...
    options: {
      title: {
        display: true,
        text: "بیشترین غایبین موجود درکلاس  ها"
      }
    }
  });
  </script>
